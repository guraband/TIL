...
- 개발 관련 도서 읽음
  - '챗GPT로 만드는 주식 & 암호화폐 자동매매 시스템'
    ...
    - ChatGPT + 파이썬으로 자동매매 프로그램을 만드는 방법을 다룬 책. 생각보다 두께가 얇아서 놀랐다. 내용 자체는 흥미로웠지만 프로그램을 모르는 사람들을 대상으로 쓴 책이라 프로그래밍의 깊이는 얕고 chatGPT 프롬프트에 대한 내용도 별로 없었다. pykrx라는 주가정보 스크래핑 라이브러리를 알게 된 점이 그나마 소득이었다.
