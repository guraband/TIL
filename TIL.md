...
## ✏️ 2026.04.27
> **🤖 토이프로젝트 개발 : 야수의 심장**
>   - 매수 ACK 알림·KST 알림 시각·stale broker order residue 정리를 보강하고 테스트·CHANGELOG·main push까지 마쳤다.
>   - 장중 websocket 복구와 마이너스 진입 로그를 점검하고 TP 성공/실패군 비교로 진입 품질 필터를 도출했다.
>   - TP/손절/timeout/eod를 로그에서 구분하도록 장마감 복기 cron 지시를 보강하고 노션 투자일지·전략 문서에 반영했다.
...
>   - 매수 대상 종목 탐색을 비효율적으로 하고 있는 것 같아서 (종목별로 순회하며 최근 ohlc 정보를 사용하는듯) KIS의 가격 상승, 거래량 상승 순위 조회 api를 사용하는 것으로 변경하고 추천 대상을 코스피200에서 코스피 전종목과 코스닥까지 포함하도록 변경했다. 추천 종목 중 코스닥의 비율은 30% 이내로 유지하도록 제한을 뒀다.
>   - 아직 보완할 점이 많지만 초단타만으로는 수익이 좋지 않아서 (현재 -4%) 포트의 절반은 ML 추천 스윙 종목으로 구성하는 것도 고려중이다.
...
>   - Open spike와 intraday 모델은 사용하지 않으려고 한다. 갭상승과 swing 위주로 테스트 하려고 함
